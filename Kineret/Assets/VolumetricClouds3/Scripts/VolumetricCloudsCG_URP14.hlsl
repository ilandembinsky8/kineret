// URP 14 / Unity 2022.3 conversion
#ifndef VOLUMETRIC_CLOUDS_URP_INCLUDED
#define VOLUMETRIC_CLOUDS_URP_INCLUDED
...
#define UNROLL_LOOP ;
#endif
TEXTURE2D(_PerlinNormalMap); SAMPLER(sampler_PerlinNormalMap);
TEXTURE2D(_CoverageMap); SAMPLER(sampler_CoverageMap);
CBUFFER_START(UnityPerMaterial)
float _Tiling,_Coverage,_CoverageLow,_CoverageHigh,_SphereHorizonStretch; float4 _PerlinNormalMap_ST,_CoverageMap_ST,_CoverageChannel;
float4 _WindDirection,_CloudTransform,_CloudSpherePosition; float _CloudBaseFlatness,_CloudFlatness,_CloudLocalScale; float4 _TimeEditor; float _TimeMult,_TimeMultSecondLayer;
float4 _VisiblePointLights[64],_VisiblePointLightsColor[64]; int _VisibleLightCount; float4 _BaseColor,_Shading,_ShadowColor,_SkyLightAttenuation;
float _SilverLining,_IndirectOcclusion,_IndirectOcclusionNormalized,_DirectContribution,_NormalsIntensity,_Normalized,_IndirectContribution,_IndirectSaturation,_AlphaCut,_DistanceBlend,_BlendWithShadowmap;
float4x4 _ToWorldMatrix; int _MaxSteps; float _StepSize,_StepNearSurface,_StepSkip,_StepParallelMult,_CloudDensity,_LodBase,_LodOffset,_DrawDistance; int _ShowStepCount; float _OrthographicPerspective;
CBUFFER_END
struct CloudLight{f3 color;f3 dir;}; struct VOUT{pf4 pos:SV_POSITION;pf4 clipPos:TEXCOORD0;pf3 eyeVec:TEXCOORD1;pf3 planePos:TEXCOORD2;pf3 ray:TEXCOORD3;}; static CloudLight light;
...
{
    VOUT o=(VOUT)0; float3 worldPos=TransformObjectToWorld(vertex.xyz);
    o.pos=TransformWorldToHClip(worldPos); o.clipPos=o.pos; o.planePos=worldPos;
    o.ray=TransformWorldToView(worldPos); o.ray*=pf3(-1,-1,1);
    o.eyeVec=lerp(worldPos-GetCameraPositionWS(),TransformViewToWorldDir(float3(0,0,-1), true),_OrthographicPerspective*UNITY_MATRIX_P[3][3]);
    return o;
}
...
inline f1 SampleClouds2DNoise(f2 UV, f1 lod, out f4 cloudTexture, out f4 cloudTexture2, out f1 coverage)
{
	...
	cloudTexture = SAMPLE_TEXTURE2D_LOD(_PerlinNormalMap, sampler_PerlinNormalMap, newUV, lod);
	cloudTexture2 = SAMPLE_TEXTURE2D_LOD(_PerlinNormalMap, sampler_PerlinNormalMap, newUV2, lod);

	coverage = _Coverage;

#if _COVERAGEMAP
	f1 coverageMap = length(SAMPLE_TEXTURE2D_LOD(_CoverageMap, sampler_CoverageMap, (UV+_CoverageMap_ST.zw)/_CoverageMap_ST.xy, lod) * _CoverageChannel);
...
#define SAMPLE_GI(v3, lod) SampleSH(normalize(v3))
...
half4 fragForwardClouds(VOUT i):SV_Target
{
    light=(CloudLight)0; Light mainLight=GetMainLight(); light.color=mainLight.color; light.dir=mainLight.direction;
    i.eyeVec=normalize(i.eyeVec); i.ray*=(_ProjectionParams.z/max(abs(i.ray.z),1e-5)); f3 planePos=i.planePos;
    f2 screenUV=i.clipPos.xy/i.clipPos.w; screenUV=screenUV*.5+.5;
    #if UNITY_UV_STARTS_AT_TOP
    if(_ProjectionParams.x<0) screenUV.y=1.0-screenUV.y;
    #endif
    pf1 rawDepth=SampleSceneDepth(screenUV); pf1 eyeDepth=LinearEyeDepth(rawDepth,_ZBufferParams);
    pf3 viewPos=i.ray*(eyeDepth/max(abs(i.ray.z),1e-5)); pf3 worldPos=TransformViewToWorld(viewPos);
    if(rawDepth>=.999999) worldPos=GetCameraPositionWS()+normalize(worldPos-GetCameraPositionWS())*100000000.0;
    f1 sceneDist=distance(planePos,worldPos);
    #if (X_AXIS)
    i.eyeVec=i.eyeVec.yxz; planePos=planePos.yxz; light.dir=light.dir.yxz; worldPos.xyz=worldPos.yxz;
    #elif (Z_AXIS)
    i.eyeVec=i.eyeVec.xzy; planePos=planePos.xzy; light.dir=light.dir.xzy; worldPos.xyz=worldPos.xzy;
    #endif
    f4 clouds=0,shadows=0;
    #if !(_RENDERSHADOWSONLY)
    clouds=RaymarchClouds(i.eyeVec,planePos,sceneDist);
    #endif
    #if (_RENDERSHADOWS)
    f3 sceneColor=SampleSceneColor(screenUV); shadows=RaymarchShadow(worldPos,planePos,clouds.a); shadows.rgb*=sceneColor;
    #endif
    f4 final=0; final=BlendFrontToBack(final,clouds); final=BlendFrontToBack(final,shadows); final=ResolveBlending(final);
    if(final.a<=_AlphaCut) discard; return final;
}
