...
namespace GISTech.GISTerrainLoader
{
    public class GISTerrainLoaderSplatMapping
    {
        ...
        public static void SetTerrainSpaltMap(GISTerrainLoaderPrefs Prefs,GISTerrainTile terrain)
        {
            MergeRaduis = Prefs.MergeRadius;

            AddLayerToTerrain(Prefs.BaseTerrainLayers, terrain);

            AddLayersToTerrain(Prefs.TerrainLayers, terrain);
            ...
            for (int y = 0; y < alphamapRwidth; y++)
            {
                for (int x = 0; x < alphamapRwidth; x++)
                {
                    ...
                    if (InterpolatedN.y < Prefs.Slope)
                    {
                    ...
                    else
                    {
                        ...
                        {
                            if (InterpolatedFactor >= Prefs.TerrainLayers[i].X_Height && InterpolatedFactor <= Prefs.TerrainLayers[i].Y_Height)
                            {
            ...
            for(int i=0;i<= Prefs.MergingFactor; i++)
            {
                MergeTerrainLayers(terrain);
            }
           
        }
        private static void AddLayersToTerrain(List<GISTerrainLoaderTerrainLayer> terrainLayers, GISTerrainTile terrain)
        {
            foreach (var layer in terrainLayers)
            {
                ...
                if (layer.Diffuse != null)
                {
                    ...
                    {
                        string path = Path.Combine(terrain.container.DestinationTerrainPath, layerName);
        ...
        private static void AddLayerToTerrain(GISTerrainLoaderTerrainLayer terrainLayer, GISTerrainTile terrain)
        {
        ...
        private static void MergeTerrainLayers(GISTerrainTile terrain)
        {
