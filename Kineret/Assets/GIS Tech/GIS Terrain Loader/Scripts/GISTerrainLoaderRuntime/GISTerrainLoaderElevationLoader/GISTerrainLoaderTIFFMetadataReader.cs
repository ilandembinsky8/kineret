...
namespace GISTech.GISTerrainLoader
{
    public class GISTerrainLoaderTIFFMetadataReader
    {
        ...
        public EPSGCodeReader Epsg2d { get; set; } = new EPSGCodeReader();
        public EPSGCodeReader Epsg3d { get; set; } = new EPSGCodeReader();
        ...
        public RasterType RasterType { get; set; } = RasterType.PixelIsPoint;
        public ModelType ModelType { get; set; } = ModelType.Projected;

        private const TiffTag GeoKeyDirectoryTag = (TiffTag)ExtraTiffTag.GeoKeyDirectoryTag;
        public string GEO_GCITATION { get; private set; }
        ...
        public string GT_CITATION { get; private set; }

        public int EPSG_Code = 0;

        public GISTerrainLoaderTIFFMetadataReader(Tiff tiff, ProjectionMode m_projectionMode)
        {
        ...
        private void ReadGeoKeyDirectory(Tiff tiff)
        {
            ...
            FieldValue[] field = new FieldValue[0];

            field = tiff.GetField(GeoKeyDirectoryTag);

            FieldValue[] m_field = new FieldValue[0];
            m_field = tiff.GetField(GeoKeyDirectoryTag);

            GeoKeyInfo TiffInfo = new GeoKeyInfo();
            if (field != null && field.Length == 2 && field[0].ToInt() >= 4)
            {
                ...
                if (NumberOfKeys > 0)
                {
                    ...
                    for (ushort i = 0; i < NumberOfKeys; i++)
                    {
                        ...
                        TiffInfo.TIFFTagLocation = shorts[keyOffset + 1];
                        TiffInfo.Count = shorts[keyOffset + 2];
                        TiffInfo.ValueOffset = shorts[keyOffset + 3];

                        switch (keyID)
                        {
                            case KeyID.GTModelTypeGeoKey:
                                GTModelTypeGeoKey = (ModelType)TiffInfo.GetValue();
                                break;
                            case KeyID.GTRasterTypeGeoKey:
                                GTRasterTypeGeoKey = (RasterType)TiffInfo.GetValue();
                                break;
                            case KeyID.GT_CITATION:
                                GT_CITATION = TiffInfo.GetString();
                                break;
                            case KeyID.GeographicTypeGeoKey:
                                GeographicTypeGeoKey = (GeographicCoordinateSystem)TiffInfo.GetValue();
                                break;
                            case KeyID.GEO_GCITATION:
                                GEO_GCITATION = TiffInfo.GetString();
                                break;
                            case KeyID.GeogGeodeticDatumGeoKey:
                                GeogGeodeticDatumGeoKey = (GeodeticDatum)TiffInfo.GetValue();
                                break;
                            case KeyID.GeogPrimeMeridianGeoKey:
                                GeogPrimeMeridianGeoKey = (PrimeMeridian)TiffInfo.GetValue();
                                break;
                            case KeyID.GeogLinearUnitsGeoKey:
                                LinearUnit = (LinearUnit)TiffInfo.GetValue();
                                break;
                            case KeyID.GeogLinearUnitSizeGeoKey:
                                GeogLinearUnitSize = TiffInfo.GetDouble();
                                break;
                            case KeyID.GeogAngularUnitsGeoKey:
                                AngularUnit = (AngularUnit)TiffInfo.GetValue();
                                break;
                            case KeyID.GeogAngularUnitSizeGeoKey:
                                AngularUnitSize = TiffInfo.GetDouble();
                                break;
                            case KeyID.GeogEllipsoidGeoKey:
                                Ellipsoid = (Ellipsoid)TiffInfo.GetValue();
                                break;
                            case KeyID.GeogSemiMajorAxisGeoKey:
                                SemiMajorAxis = TiffInfo.GetDouble();
                                break;
                            case KeyID.GeogSemiMinorAxisGeoKey:
                                SemiMinorAxis = TiffInfo.GetDouble();
                                break;
                            case KeyID.GeogInvFlatteningGeoKey:
                                InvFlattening = TiffInfo.GetDouble();
                                break;
                            case KeyID.GeogAzimuthUnitsGeoKey:
                                AzimuthUnit = (AngularUnit)TiffInfo.GetValue();
                                break;
                            case KeyID.GeogPrimeMeridianLongGeoKey:
                                PrimeMeridianLongitude = TiffInfo.GetDouble();
                                break;
                            case KeyID.EPSG_CODE:
                                ProjectedCoordinatesystem = (ProjectedCoordinateSystem)TiffInfo.GetValue();
                                EPSG_Code = (int)ProjectedCoordinatesystem;
                                break;
                            ...
                            case KeyID.ProjLinearUnitSizeGeoKey:
                                ProjLinearUnitSize = TiffInfo.GetDouble();
                                break;
            ...
            if (projectionMode == ProjectionMode.Geographic)
            {
                GTModelTypeGeoKey = ModelType.Projected;
                GTModelTypeGeoKey = ModelType.Geographic;
                EPSG_Code = 4326;
            }
            else
            {
                if (projectionMode == ProjectionMode.AutoDetection)
                {
                    if (m_field != null)
                    {
                        if (m_field.Length >= 1)
                        {
                            int m_count = m_field[0].ToInt();
                            var m_bytes = m_field[1].ToByteArray();
                            ushort[] m_shorts = m_bytes.ConvertToArray(m_count, sizeof(ushort), BitConverter.ToUInt16);

                            foreach (ushort s in m_shorts)
                            {
                                if (EPSG_Code == 0 && s == 4326)
                                {
                                    GTModelTypeGeoKey = ModelType.Projected;
                                    GTModelTypeGeoKey = ModelType.Geographic;
                                    EPSG_Code = 4326;
                ...
                {
                    ReadGeoTiffTags(tiff.GetField(GeoKeyDirectoryTag));
                    ProjectedCoordinatesystem = (ProjectedCoordinateSystem)EPSG_Code;
                }
            }

            if (LinearUnit == LinearUnit.Undefined)
            {
                var ProjLinearUnits = tiff.GetField((TiffTag)ExtraTiffTag.ProjLinearUnitsGeoKey);

                if (ProjLinearUnits != null)
                {
                    if (ProjLinearUnits.Length >= 1)
                    {
                        int m_count = ProjLinearUnits[0].ToInt();
                        var m_bytes = ProjLinearUnits[1].ToByteArray();
                        ushort[] m_shorts = m_bytes.ConvertToArray(m_count, sizeof(ushort), BitConverter.ToUInt16);
                        LinearUnit = (LinearUnit)m_shorts[0];
            ...
            if (GTModelTypeGeoKey == ModelType.Projected)
            {
                ...
                {
                    UnityEngine.Debug.LogError("LinearUnit '" + LinearUnit + "' not supported");
                }
            }
        ...
        private void SetProjection()
        {
            ...
            if (GTModelTypeGeoKey == ModelType.Projected)
            {
                CoordinateReferenceSystem = new GISTerrainLoaderProjectionSystem((int)ProjectedCoordinatesystem, GEO_GCITATION);
                ProjectionSystem = ProjectionSystem.defined;
            }

        }

        private void ReadGeoTiffTags(FieldValue[] geoKeyDirectoryTag)
        {
                ...
                foreach (var fieldValue in geoKeyDirectoryTag)
                {
                    ...
                    for (int i = 0; i < sourceArray.Length; i += 8)
                    {
                        ...
                        switch (tag)
                        {
                            ...
                            case 1024:
                                {
                                    ModelType = (ModelType)roci;
                                    break;
                                }
                            case 1025:
                                {
                                    RasterType = (RasterType)roci;
                                    break;
                                }
                            case 2048:
                                {
                                    Epsg2d.SetCodeString(roci.ToString());
                                    EPSG_Code = int.Parse(Epsg2d.CodeString);
                                    break;
                                }
                            case 4096:
                                {
                                    Epsg3d.SetCodeString(roci.ToString());
                                    EPSG_Code = int.Parse(Epsg3d.CodeString);
                                    break;
                                }
                            default:
                                break;
    ...
    public class ExtraTiffTag
    {
        ...
        public const ushort ProjLinearUnitsGeoKey = 3076;
    }
    ...
    public enum KeyID : ushort
    {
        ...
        EPSG_CODE = 3072, //ProjectedCSTypeGeoKey
