...
namespace GISTech.TerrainStreaming
{
    ...
    public static class TerrainStreamingMathIntersections
    {
        ...
        {
            double epsilon = TerrainStreamingMathUtility.EPSILON;
        ...
        public static bool LinePlane(DVector2 planePos, DVector2 planeNormal, DVector2 line_p1, DVector2 line_p2)
        {
            ...
            if (denominator > epsilon || denominator < -epsilon)
            {
                ...
                if (TerrainStreamingGeometry.IsPointBetweenPoints(line_p1, line_p2, intersectionPoint))
                {
                    areIntersecting = true;
                }
            }
        ...
        public static bool PlanePlane(DVector2 planePos_1, DVector2 planeNormal_1, DVector2 planePos_2, DVector2 planeNormal_2)
        {
            ...
            float one = 1f - TerrainStreamingMathUtility.EPSILON;
        ...
        public static bool PointTriangle(TerrainStreamingTriangle2 t, DVector2 p, bool includeBorder)
        {
            float epsilon = TerrainStreamingMathUtility.EPSILON;
        ...
        public static bool IsTriangleInsideTriangle(TerrainStreamingTriangle2 t1, TerrainStreamingTriangle2 t2)
        {
            bool isWithin = false;
        ...
        public static bool AABB_AABB_2D(TerrainStreamingAABB2 r1, TerrainStreamingAABB2 r2)
        {
        ...
        {
            DVector2 circleCenter = TerrainStreamingGeometry.CalculateCircleCenter(a, b, c);
            ...
            if (distPointCenterSqr < radiusSqr - TerrainStreamingMathUtility.EPSILON * 2f)
            {
                return IntersectionCases.IsInside;
            }
            else if (distPointCenterSqr > radiusSqr + TerrainStreamingMathUtility.EPSILON * 2f)
            {
        ...
        public static bool PointPolygon(List<DVector2> polygonPoints, DVector2 point)
        {
            ...
            for (int i = 0; i < polygonPoints.Count; i++)
            {
                ...
                int iPlusOne = TerrainStreamingMathUtility.ClampListIndex(i + 1, polygonPoints.Count);

                DVector2 l2_p2 = polygonPoints[iPlusOne];

                if (TerrainStreamingMathIntersections.LineLine(l1_p1, l1_p2, l2_p1, l2_p2, shouldIncludeEndPoints: true))
                {
        ...
        public static bool TriangleTriangle2D(TerrainStreamingTriangle2 t1, TerrainStreamingTriangle2 t2, bool do_AABB_test)
        {
            bool isIntersecting = false;

            if (do_AABB_test)
            {
                TerrainStreamingAABB2 r1 = new TerrainStreamingAABB2(t1.MinX(), t1.MaxX(), t1.MinY(), t1.MaxY());

                TerrainStreamingAABB2 r2 = new TerrainStreamingAABB2(t2.MinX(), t2.MaxX(), t2.MinY(), t2.MaxY());
