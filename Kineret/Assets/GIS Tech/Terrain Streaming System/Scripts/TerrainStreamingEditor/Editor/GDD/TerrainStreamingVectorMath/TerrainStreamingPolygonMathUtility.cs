...
namespace GISTech.TerrainStreaming
{
    ...
    public static class TerrainStreamingPolygonMathUtility
    {
        public static List<List<DVector2>> ClipPolygons(List<DVector2> polyVector2, List<DVector2> clipPolyVector2, BooleanOperation booleanOperation)
        {
            ...
            for (int i = 0; i < poly.Count; i++)
            {
                ...
                int iPlusOne = TerrainStreamingMathUtility.ClampListIndex(i + 1, poly.Count);
                ...
                {
                    int jPlusOne = TerrainStreamingMathUtility.ClampListIndex(j + 1, clipPoly.Count);

                    DVector2 c = clipPoly[j].coordinate;

                    DVector2 d = clipPoly[jPlusOne].coordinate;

                    if (TerrainStreamingMathIntersections.LineLine(a, b, c, d, true))
                    {
                        hasFoundIntersection = true;

                        DVector2 intersectionPoint2D = TerrainStreamingMathIntersections.GetLineLineIntersectionPoint(a, b, c, d);
        ...
        private static bool IsPolygonInsidePolygon(List<DVector2> polyOne, List<DVector2> polyTwo)
        {
            bool isInside = false;

            for (int i = 0; i < polyOne.Count; i++)
            {
                if (TerrainStreamingMathIntersections.PointPolygon(polyTwo, polyOne[i]))
                {
                    isInside = true;
        ...
        private static List<ClipVertex> InitDataStructure(List<DVector2> polyVector)
        {
            ...
            {
                int iPlusOne = TerrainStreamingMathUtility.ClampListIndex(i + 1, poly.Count);
                int iMinusOne = TerrainStreamingMathUtility.ClampListIndex(i - 1, poly.Count);
        ...
        {
            bool isInside = TerrainStreamingMathIntersections.PointPolygon(clipPolyVector, poly[0].coordinate);
