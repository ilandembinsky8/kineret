...
{
    public class TerrainStreamingFileAsync
    {
        ...
        public static async Task WriteAllBytes(string path, byte[] bytes, CancellationTokenSource m_taskSource = null)
        {
            if (path == null) throw new ArgumentException("path");
            if (bytes == null) throw new ArgumentException("bytes");
            ...
            using (var fs = OpenWrite(path))
            {
                await fs.WriteAsync(bytes, 0, bytes.Length, m_taskSource.Token).CancelWith(m_taskSource.Token);
            }
        }
    }
}
